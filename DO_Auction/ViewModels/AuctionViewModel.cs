...
namespace DO_Login.ViewModels
{
    public class AuctionViewModel : ViewModelBase
    {
        ...
        public OfferModel SelectedItemOffer { get => _SelectedItemOffer; set { _SelectedItemOffer = value; RaisePropertyChanged(); } }

        //GRID VIEW ROWS
        private DataGridViewRowCollection _DataGridViewCollection;
        public DataGridViewRowCollection DataGridViewCollection { get => _DataGridViewCollection; set { _DataGridViewCollection = value; RaisePropertyChanged(); } }

        //YOU BIDS
        private BindingList<BidModel> _bids = new BindingList<BidModel>();
        public BindingList<BidModel> Bids { get => _bids; set { _bids = new BindingList<BidModel>(value); RaisePropertyChanged(); } }
        ...
        public void interval()
        {
            ...
            _timer2.Elapsed += new ElapsedEventHandler((s, e) =>
            {
                new Thread(new ThreadStart(() =>
                    {
                        api.setBid(Bids.ToList().FindAll(bid => bid.Time >= CurrentTimeServer).ToList()
                            .FindAll(bid => bid.isRunning == false).ToList()
                            .FindAll(bid => bid.YouNewBid > bid.Offer.currentBid));
                    }
                )).Start();
            });
